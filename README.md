...
# Level 1 topics
topics = ["Basics", "OOP", "Data Handling", "ML", "DSA", "Visualization"]
for t in topics:
    G.add_edge("Python", t)

# Subtopics under DSA
dsa_subtopics = ["hello", "who", "say"]
for sub in dsa_subtopics:
    G.add_edge("DSA", sub)
...
categories = {
    "Core": ["Variables", "Loops", "Functions"],
...
for cat, subs in categories.items():
    G.add_edge("Python", cat)
    for s in subs:
        G.add_edge(cat, s)


# custom hierarchy layout (no graphviz needed)
...
dot.node("A", "Python Tutorial and Reference")

dot.node("B1", "Core Language Fundamentals")
dot.node("B2", "OOP")
dot.node("B3", "File Handling")
dot.node("B4", "Modules")
dot.node("B5", "Data Analysis")
dot.node("B6", "Machine Learning")
dot.node("B7", "DSA")
dot.node("B8", "Database")
dot.node("B9", "References")
...
dot.render("python_mindmap", format="png")'''

# Nokia 108 style console UI using keyboard navigation
# Works in Windows with "pip install windows-curses"
'''import curses

# Menu items to simulate Nokia-style icons
menu_items = ["Messages", "Contacts", "Call Log", "Settings", "Games", "Extras", "Exit"]
...
def main(stdscr):
    ...
    current_index = 0  # highlight the first menu item

    while True:
        stdscr.clear()

        # Draw title
        stdscr.addstr(1, 5, "📱 NOKIA 108", curses.A_BOLD)
        stdscr.addstr(2, 2, "Use ↑ ↓ to navigate, Enter to select", curses.A_DIM)
        stdscr.addstr(3, 2, "-" * 30)

        # Draw menu
        for i, item in enumerate(menu_items):
            if i == current_index:
                stdscr.addstr(5 + i, 6, f"> {item}", curses.A_REVERSE)  # highlighted
            else:
                stdscr.addstr(5 + i, 6, f"  {item}")
        ...
        if key in [curses.KEY_UP, ord('w')]:
            ...
            if menu_items[current_index] == "Exit":
                break
            stdscr.clear()
            stdscr.addstr(8, 8, f"Opening {menu_items[current_index]}...", curses.A_BOLD)
            stdscr.refresh()
...
# ui using python tkinter and json
'''
# File: questions.json
# Save this JSON in the same folder as quiz_app.py as questions.json
...
# File: quiz_app.py
# A simple MCQ GUI using tkinter that loads questions from questions.json
# Added: larger question font and Zoom In / Zoom Out buttons
...
class QuizApp:
    def __init__(self, master, questions):
        ...
        # font size control
        self.font_size = 16  # default question font size
        ...
        # UI layout
        self.frame = tk.Frame(master, padx=16, pady=12)
        self.frame.pack(fill='both', expand=True)

        self.q_label = tk.Label(self.frame, textvariable=self.question_var, wraplength=700, justify='left', font=(None, self.font_size))
        self.q_label.pack(anchor='w', pady=(0,10))

        self.radio_buttons = []
        for i in range(4):
            rb = tk.Radiobutton(self.frame, text='', variable=self.choice_var, value=i, anchor='w', justify='left', font=(None, self.option_font_size))
        ...
        self.prev_btn = tk.Button(nav_frame, text='Previous', command=self.prev_q)
        self.prev_btn.pack(side='left')

        self.next_btn = tk.Button(nav_frame, text='Next', command=self.next_q)
        self.next_btn.pack(side='left', padx=(8,0))

        # Zoom controls
        zoom_frame = tk.Frame(nav_frame)
        zoom_frame.pack(side='left', padx=12)

        self.zoom_in_btn = tk.Button(zoom_frame, text='Zoom +', command=self.zoom_in)
        self.zoom_in_btn.pack(side='left')

        self.zoom_out_btn = tk.Button(zoom_frame, text='Zoom -', command=self.zoom_out)
        self.zoom_out_btn.pack(side='left', padx=(6,0))

        self.submit_btn = tk.Button(nav_frame, text='Submit', command=self.submit)
    ...
    def load_question(self):
        ...
        self.question_var.set(f'Question {self.index + 1}/{self.total}: ' + q['question'])

        options = q.get('options', [])
        for i, rb in enumerate(self.radio_buttons):
            if i < len(options):
                rb.config(text=options[i], state='normal')
            else:
                rb.config(text='', state='disabled')

        # restore previous answer if any
        saved = self.user_answers[self.index]
        self.choice_var.set(saved if saved is not None else -1)

        # update navigation buttons
        self.prev_btn.config(state='normal' if self.index > 0 else 'disabled')
        self.next_btn.config(state='normal' if self.index < self.total - 1 else 'disabled')

        # apply font sizes (in case zoom changed)
        self.apply_fonts()

    def apply_fonts(self):
        # update question label font
        self.q_label.config(font=(None, self.font_size))
        # update radio button fonts
        for rb in self.radio_buttons:
            rb.config(font=(None, self.option_font_size))

    def save_answer(self):
        choice = self.choice_var.get()
        if choice >= 0:
            self.user_answers[self.index] = choice
        else:
            self.user_answers[self.index] = None
    ...
    def zoom_in(self):
        # increase both question and option font sizes
        if self.font_size < 40:
            self.font_size += 2
        if self.option_font_size < 30:
            self.option_font_size += 1
        self.apply_fonts()

    def zoom_out(self):
        if self.font_size > 8:
            self.font_size -= 2
        if self.option_font_size > 8:
            self.option_font_size -= 1
        self.apply_fonts()
    ...
    def submit(self):
        ...
        if None in self.user_answers:
            if not messagebox.askyesno('Confirm', 'Some questions are unanswered. Do you still want to submit?'):
                return
        ...
        for i, q in enumerate(self.questions):
            ...
            if ans is None:
                continue
            if self.user_answers[i] == ans:
                score += 1

        percent = score * 100 / self.total if self.total else 0
        ...
        for i, q in enumerate(self.questions):
            ...
            user_text = q["options"][user_ans] if user_ans is not None else "Not answered"
        ...
        messagebox.showinfo(
            "Result",
            f"Your score is {score} out of {self.total}\n"
...
def main():
    ...
    # ask user to choose a questions file
    messagebox.showinfo('Load questions', 'Please select the questions JSON file. A sample questions.json is provided.')
    path = filedialog.askopenfilename(title='Select questions.json', filetypes=[('JSON files','*.json'), ('All files','*.*')])
    if not path:
    ...
    try:
        ...
        messagebox.showerror('Error', f'Failed to load questions file.{e}')
        root.destroy()
        return

    if not questions:
        messagebox.showerror('Error', 'No valid questions found in the file. Exiting.')
        root.destroy()
        return
...
'''def hide_card(card_number):
    ...
    # mask length
    masked_part = "*" * (len(cleaned) - 4)
    
    # format in groups of 4
    formatted = " ".join([masked_part[i:i+4] for i in range(0, len(masked_part), 4)])
    
    return (formatted + " " + last_four).strip()

# Example
number = input("Enter your credit card number: ")
print("Hidden number:", hide_card(number))'''


# is pronic number such as 2,6,12 etc
'''def is_pronic(num):
...
# Broken Keyboard
'''def can_type(word, working_keys):
    working = set(working_keys.lower())
    for ch in word.lower():
        if ch not in working:
            return False
    return True


working_keys = input("Enter working keys: ")
word = input("Enter the word to type: ")

if can_type(word, working_keys):
    print("You can type the word")
else:
    print("You cannot type the word")'''

#trai n bill board
'''import tkinter as tk

# Text in 3 languages (all in one horizontal line)
messages = [
    "Train Name Rajdhani Express   Train No 12301   Departure 17 30",
    "ट्रेन का नाम राजधानी एक्सप्रेस   ट्रेन संख्या 12301   प्रस्थान 17 30",
    "ট্রেনের নাম রাজধানী এক্সপ্রেস   ট্রেন নম্বর 12301   প্রস্থান 17 30"
]
...
root = tk.Tk()
root.title("Train Display")

frame = tk.Frame(root, bg="black", padx=30, pady=30)
frame.pack(fill="both", expand=True)

label = tk.Label(frame, text="", font=("Arial", 26), fg="white", bg="black")
label.pack()

change_text()
root.mainloop()'''

#update version of train dash board

'''import tkinter as tk

# --- Train data in 3 languages ---

english = [
    "34612 Baruipur Local       Departure 10 15",
    "34813 Diamond Harbour      Departure 11 05",
    "34414 Sonarpur Local       Departure 12 20",
    "34715 Lakshmikantapur      Departure 13 40"
]

hindi = [
    "34612 बारुईपुर लोकल       प्रस्थान 10 15",
    "34813 डायमंड हार्बर       प्रस्थान 11 05",
    "34414 सोनारपुर लोकल       प्रस्थान 12 20",
    "34715 लक्ष्मीकांतपुर       प्रस्थान 13 40"
]

bengali = [
    "34612 বারুইপুর লোকাল       প্রস্থান 10 15",
    "34813 ডায়মন্ড হারবার       প্রস্থান 11 05",
    "34414 সোনারপুর লোকাল       প্রস্থান 12 20",
    "34715 লক্ষ্মীকান্তপুর       প্রস্থান 13 40"
]
...
def update_language():
    ...
    # Change all train lines to the new language
    for i in range(len(labels)):
        labels[i].config(text=languages[lang_index][i])
    
    # Move to next language
    lang_index = (lang_index + 1) % 3
    
    # Change again after 4 seconds
    root.after(4000, update_language)


# --- GUI Setup ---

root = tk.Tk()
root.title("Train Display Board")
...
for _ in range(4):
    lbl = tk.Label(frame, text="", font=("Arial", 26), fg="white", bg="black", anchor="w")
    lbl.pack(pady=5)
    labels.append(lbl)
...
def generate_pnr():
    # simple fake PNR generator
    return str(random.randint(10**9, 10**10 - 1))

def print_ticket():
    train_no = entry_train_no.get().strip()
    train_name = entry_train_name.get().strip()
    journey_date = entry_date.get().strip()
    source = entry_from.get().strip()
    destination = entry_to.get().strip()
    passenger_name = entry_passenger.get().strip()
    age = entry_age.get().strip()
    gender = combo_gender.get().strip()
    travel_class = combo_class.get().strip()

    if not (train_no and train_name and journey_date and source and
            destination and passenger_name and age and gender and travel_class):
...
Fare          Rs 450.00   (sample)
Status        CNF
Coach Berth   S3  32  (sample)

Thank you for travelling with us
"""

    show_ticket_window(ticket_text)

def show_ticket_window(ticket_text):
    win = tk.Toplevel(root)
    win.title("Printed Ticket")
...
# Train details
label_section1 = ttk.Label(main_frame, text="Train details", font=("Arial", 14, "bold"))
label_section1.grid(row=0, column=0, columnspan=4, pady=(0, 10))

ttk.Label(main_frame, text="Train no").grid(row=1, column=0, sticky="e", pady=5)
entry_train_no = ttk.Entry(main_frame, width=20)
entry_train_no.grid(row=1, column=1, sticky="w", pady=5)

ttk.Label(main_frame, text="Train name").grid(row=1, column=2, sticky="e", pady=5)
entry_train_name = ttk.Entry(main_frame, width=25)
entry_train_name.grid(row=1, column=3, sticky="w", pady=5)

ttk.Label(main_frame, text="Date of journey").grid(row=2, column=0, sticky="e", pady=5)
entry_date = ttk.Entry(main_frame, width=20)
entry_date.insert(0, "21/11/2025")  # sample
entry_date.grid(row=2, column=1, sticky="w", pady=5)

ttk.Label(main_frame, text="From").grid(row=2, column=2, sticky="e", pady=5)
entry_from = ttk.Entry(main_frame, width=20)
entry_from.grid(row=2, column=3, sticky="w", pady=5)

ttk.Label(main_frame, text="To").grid(row=3, column=2, sticky="e", pady=5)
entry_to = ttk.Entry(main_frame, width=20)
entry_to.grid(row=3, column=3, sticky="w", pady=5)

# Passenger details
label_section2 = ttk.Label(main_frame, text="Passenger details", font=("Arial", 14, "bold"))
label_section2.grid(row=4, column=0, columnspan=4, pady=(15, 10))

ttk.Label(main_frame, text="Name").grid(row=5, column=0, sticky="e", pady=5)
entry_passenger = ttk.Entry(main_frame, width=25)
entry_passenger.grid(row=5, column=1, sticky="w", pady=5)

ttk.Label(main_frame, text="Age").grid(row=5, column=2, sticky="e", pady=5)
entry_age = ttk.Entry(main_frame, width=10)
entry_age.grid(row=5, column=3, sticky="w", pady=5)

ttk.Label(main_frame, text="Gender").grid(row=6, column=0, sticky="e", pady=5)
combo_gender = ttk.Combobox(main_frame, values=["Male", "Female", "Other"], width=17, state="readonly")
combo_gender.grid(row=6, column=1, sticky="w", pady=5)

ttk.Label(main_frame, text="Class").grid(row=6, column=2, sticky="e", pady=5)
combo_class = ttk.Combobox(main_frame, values=["SL", "3A", "2A", "1A", "CC"], width=17, state="readonly")
combo_class.grid(row=6, column=3, sticky="w", pady=5)

# Button
btn_print = ttk.Button(main_frame, text="Book and print ticket", command=print_ticket)
btn_print.grid(row=7, column=0, columnspan=4, pady=25)
...
def show_tree(path, prefix=""):
    ...
    for item in items:
        ...
        if os.path.isdir(full_path):
            show_tree(full_path, prefix + "    ")

print("Tree structure starting...\n")

root_path = "C:\Users\MD.AAMIR\Desktop\New folder"
print(root_path)
show_tree(root_path)
...
# ---------- USER SETTINGS ----------
phone_number = "+916291564515"   # include country code
message = "Hello, this message was sent automatically"
send_hour = 14    # 24-hour format
send_minute = 30  # minute
# -----------------------------------
...
with open("letter.text", "r", encoding="utf-8") as file:
    text = file.read()
...
It is easy to learn and widely used in many fields.
...
The language emphasizes readability.""",
    ...
    "Applications": """Python is used in web development.
It is popular in data science and AI.
Automation and scripting are also common uses."""
}

# -------- FUNCTION FOR DELAY PRINT --------
...
# -------- MAIN LOOP --------
while True:
    heading = input("Enter heading name (or type exit): ").strip()
    ...
    if heading in content:
        slow_print(content[heading])
    else:
        print("Heading not found. Try again.\n")''''''
...
# -------- MAIN PROGRAM --------
content = load_content("letter.text")
...
#ai intergrated python code if you have mony then use it
...
#Time interval music play program
...
# 2. Set the interval (25 minutes = 1500 seconds)
INTERVAL = 25
music_file = "nervent.mp3" # Make sure this is in the same folder!

print("Timer started. See you in 25 minutes!")
...
try:
    while True:
        ...
        # Optional: Let the music play for 30 seconds, then stop
        time.sleep(30)
        pygame.mixer.music.stop()
        
        print("Timer reset. Waiting for next interval...")
...
# 1. Define your playlist link
# We add '&autoplay=1' and '&mute=1' to the URL
playlist_url = "https://youtu.be/DHGwn4G6-Ws?si=avWV6ZONr1W0_Wmz"

# 2. Set your timer (in seconds)
wait_time = 10 
print(f"Waiting for {wait_time} seconds before opening the playlist...")
...
# ---------- Global ----------
DICTIONARY_FILE = "word.txt"  # put your dictionary file here
...
def find_all_words(name_letters, dictionary):
    valid = []
    for word in dictionary:
        if can_make(word, name_letters):
            valid.append(word)
    return valid


# ---------- Process choice function ----------
def process_choice(choice, available_letters, phrase):
    phrase.append(choice)
    remaining = Counter(available_letters) - Counter(choice)
...
def main():
    ...
    while True:
        print("\n--- Phrase Anagram Generator ---")
        name = input("Enter a name or phrase: ").strip()
        name_letters = clean_text(name)

        phrase = []
        limit = len(name_letters)

        while len("".join(phrase)) < limit:
            print("\nCurrent phrase:", " ".join(phrase))
            print("Remaining letters:", name_letters)

            words = find_all_words(name_letters, dictionary)
            ...
            if 1 <= choice <= len(words[:20]):
                selected_word = words[choice - 1]
                name_letters = process_choice(
                    selected_word, name_letters, phrase
                )
            else:
                print("Invalid choice. Try again.")
...
# 1. Define the x-values (from -10 to 10, with 400 points for smoothness)
x = np.linspace(-10, 10, 400)

# 2. Define the quadratic equation: y = x^2 - 2x + 1
y = x**2 - 2*x + 1

# 3. Create the plot
plt.figure(figsize=(8, 5))
plt.plot(x, y, label='$y = x^2 - 2x + 1$', color='blue', linewidth=2)

# 4. Add math-friendly formatting
...
# 3. Define the function Z = X^2 + Y^2 (The Paraboloid)
Z = X**2 + Y**2

# 4. Plot the surface
surface = ax.plot_surface(X, Y, Z, cmap='viridis', edgecolor='none')

# 5. Add labels and a color bar
...
MP3_FOLDER = r"C:\osers\MD.AAMIR\PycharmProjects\pythonProject4" # remember to put user isted of oser

# input multiple names separated by space
user_input = input("Enter song names (space separated): ").strip().lower()

# split input into list
song_names = user_input.split()
...
for name in song_names:
    ...
    if name in mp3_files:
        song_path = os.path.join(MP3_FOLDER, mp3_files[name])
        print("Playing:", mp3_files[name])
...
def interactive_pdf_reader(file_path):
    try:
        with pdfplumber.open(file_path) as pdf:
            ...
            while True:
                user_input = input("Enter page number to read (or type 'stop' to quit): ").lower().strip()

                # 3. Check if user wants to stop
                if user_input == "stop":
                    print("\n👋 Happy reading! Program closed.")
                    break

                # 4. Try to process the page number
                try:
                    ...
                    if 1 <= page_num <= total_pages:
                        ...
                        if text:
                            ...
                            print("[This page appears to be an image or contains no selectable text.]")
                        
                        print("\n" + "="*50 + "\n")
                    else:
                        print(f"⚠️  Please enter a number between 1 and {total_pages}.\n")
                
                except ValueError:
                    print("⚠️  Invalid input! Please enter a page number or 'stop'.\n")
...
# Usage - Ensure this filename is exactly correct
filename = "48_Hour_Start_up_From_idea_to_launch_in_1_weekend.pdf"
interactive_pdf_reader(filename)'''
    ...
    with open(filename, "r") as file:
        for line in file:
            ...
            if line:  # ignore empty lines
                bus, route = line.split(":", 1)
                routes[bus.strip().upper()] = route.strip()
    
    return routes


def search_bus_route(routes):
    bus_number = input("Enter bus number: ").upper()
    
    if bus_number in routes:
        print("Route for", bus_number, "is:")
        print(routes[bus_number])
    else:
        print("Bus number not found.")


# Main program
routes = load_routes("bus_routes.txt")
search_bus_route(routes)'''